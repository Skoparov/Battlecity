...
static constexpr auto tag_base_health = "BaseHealth";
...
void game_settings::set_base_health( const uint32_t health ) noexcept
{
    m_base_health = health;
}

uint32_t game_settings::get_base_health() const noexcept
{
...
game_settings read_game_settings( const QString& file )
{
    ...
    while( !xml_reader.atEnd() && !xml_reader.hasError() )
    {
        ...
        if(token == QXmlStreamReader::StartElement)
        {
            ...
            if( name == tag_base_health )
            {
                settings.set_base_health( xml_reader.readElementText().toUInt() );
            }
